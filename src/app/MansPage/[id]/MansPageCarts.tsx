...
export default function MansPageCarts(): JSX.Element {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `http://localhost:4000/api/ManProduct/${id}`,
          { headers: { "Content-Type": "application/json" } }
        )
        
        setData(res.data.Man.products)
        console.log(res.data.Man.category)
  ...
  return (
    <div className="min-h-screen w-full bg-white/40 backdrop:backdrop-blur-2xl px-4 py-10">
  ...
  <img
    ...
    alt={item.title}
    className="object-cover h-full w-full p-2  rounded-2xl"
  />
</div>
